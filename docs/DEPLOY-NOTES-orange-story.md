# 배포 노트 — Orange Story 리브랜딩 (v0.20.0 → v0.21.0)

> 서버에서 `bash scripts/deploy.sh`를 돌리기 **전에** 읽는다.
> 이 배포는 운영보다 커밋 20개 앞서 있고 미적용 마이그레이션이 3개다. 그중 하나가
...
운영은 `bcef157`(v0.20.0) — 이 노트를 처음 쓸 때의 `dfb2fdd`(v0.19.1) 에서
`1cc7985`(리워드 요율·업로드 한도) 하나가 더 배포된 상태다. 그 커밋은 이 갈래가
지우는 `reward.py` 를 고치는 것이라 `9a9bbb5` 머지에서 걷어냈고, 마이그레이션은
추가하지 않아 DB 는 여전히 `d953acd3a18d` 다 — 아래 expand/contract 분석은 그대로
유효하다. 롤백 대상 슬롯의 코드도 `reward_points` 를 참조하는 점이 같다.
...
> **이 절은 Google 로그인에만 해당한다.** 라이트닝(LNURL-auth)은 세션이 아니라 신원
> 자체가 도메인에서 파생되므로 성격이 다르고 더 나쁘다 — 아래 별도 절을 보라.
...
### 라이트닝 로그인은 세션이 아니라 신원이 갈라진다 (실제로 터졌다)

LUD-04 는 지갑의 linkingKey 를 `HMAC-SHA256(hashingKey, FQDN)` 으로 파생한다. 도메인이
바뀌면 같은 지갑이 관계 없는 다른 공개키를 내놓고, `auth.py` 의 `oauth_sub` 조회가
빗나가 **에러 없이 빈 새 계정이 생긴다.** 사용자는 로그인 실패가 아니라 "기록이 전부
사라진 낯선 계정"을 본다. 서버에는 구 키와 신 키를 이어붙일 방법이 없다.

2026-08-30 전환 다음날 중복 계정 4개가 실제로 생겼다. 대응은 `LNURL_BASE_URL` 을
`stackhealth.life` 로 고정하고 구 도메인의 `/api/v1/auth/lnauth` 를 301 예외로 두는
것이다. 원인·피해 범위·적용 구조·남은 과제는 `docs/LNURL-DOMAIN-MIGRATION.md`.
