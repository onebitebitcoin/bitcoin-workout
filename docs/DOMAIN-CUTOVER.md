...
카테고리 리맵(`004aa221ea8d`)이 따로 만드는 증상은 하나뿐이고, 그마저도 크래시는 아니다. 프론트엔드는 4단계 전에 이미 새 빌드(비트코인/일상 2종)로 교체돼 있어 신규 페이지 로드에는 영향이 없다. 다만 배포 전부터 열려 있던 브라우저 탭은 구 프론트엔드 번들을 그대로 실행 중인데, `frontend/src/pages/upload/StepMeta.tsx:9`의 `MAIN_CATEGORIES = ['가벼운 활동', '땀 흘리는 운동']` 하드코딩 목록이 그 안에 살아있다. 리맵 후 `post.tags[0]`은 `'일상'`이 되어 이 목록에 더는 없으므로, `PostEditPage.tsx:46`의 `includes(tags[0]) ? tags[0] : null` 판정이 `null`로 떨어진다. 그 탭에서 게시물 수정 화면을 열면 메인 카테고리 선택이 조용히 풀려서 보이는 정도다. 저장을 누르면 위 표의 `PATCH .../posts/{id}` 경로를 타고 결국 500으로 끝나고, 새로고침하면 사라지는 증상이라 영향은 작다.

**창의 길이**

스크립트만 놓고 보면 하한은 4단계에서 DROP 문이 실행되는 순간이고, 상한은 7단계에서 `sleep 5` 뒤 `CURRENT_SLOT`이 죽는 순간이다. 그 사이에 세 구간이 순서대로 더해진다: 카테고리 리맵이 `posts`/`challenges`를 행 단위 Python 루프로 순회하며 UPDATE하는 나머지 시간(개별 UPDATE를 반복하는 구조라 DROP 자체보다 오래 걸릴 수 있다), 5단계에서 OpenCV 같은 무거운 의존성을 새 프로세스가 다시 로드하는 재시작 시간, 6단계의 헬스체크 대기(2초 간격, 최대 60초). 여기에 7단계의 고정 5초가 붙는다. **확인 필요**: 정확한 초 단위 값은 운영 DB의 `posts` 행 수와 서버 자원에 따라 달라져 리포지토리만으로는 알 수 없다 — 실제 배포 로그에서 [4/8] 시작부터 [7/8]의 "이전 슬롯 종료" 줄까지 타임스탬프 차이를 재보는 쪽이 정확하다. 최선의 경우에도 수십 초, 리맵 대상 데이터가 많거나 새 프로세스 기동이 느리면 1분을 넘길 수 있는 구간으로 봐야 한다.
...
**B안 실측 검증 — PostgreSQL 임시 DB, 실제 코드로 확인**

B안을 택했을 때 실제로 문제가 없는지는 직접 실행해서 확인했다. 로컬 PostgreSQL(homebrew, 16)에 임시 DB를 만들고, `c7d8e9f0a1b2`를 마이그레이션 체인에서 뺀 뒤 `004aa221ea8d`의 `down_revision`을 `d953acd3a18d`로 되돌린 사본을 별도 `alembic.ini`로 가리켜 실행했다. 원본 마이그레이션 파일과 `backend/alembic/`은 손대지 않았고, 임시 DB·프로세스·스크립트는 검증 후 전부 지웠다(`git status`로 `backend/alembic/`이 이전과 동일함을 확인했다).

1. **마이그레이션 체인**: `alembic upgrade head`가 에러 없이 끝났고 `alembic heads`는 `004aa221ea8d` 단일 head로 나왔다. 체인은 성립한다.
2. **새 코드가 reward_points 있는 DB에서 도는가**: 실제 앱(uvicorn)을 이 DB에 붙여 기동했더니 `/health`가 `{"status":"ok"}`로 정상 응답했다. SQLAlchemy 모델에 없는 잔존 테이블은 무해했다. 다만 `backend/tests/`의 pytest는 `conftest.py`가 자체 인메모리 SQLite(`Base.metadata.create_all`)를 쓰고 `DATABASE_URL`을 참조하지 않아, 이 임시 DB로 돌린 게 아니다 — 별도로 `pytest`만 실행해 330개 전부 통과(커버리지 79%)한 건 확인했지만, 이 항목 자체를 검증하는 실행은 아니다.
3. **관리자 유저 삭제 — 확인했다, FK 위반이 실제로 난다**: `users` 행 하나를 만들고 `reward_points`에 그 `user_id`를 참조하는 행을 하나 심은 뒤, 실행 중인 서버에 `DELETE /api/v1/admin/users/{id}`를 `X-Admin-Key`로 그대로 태웠다. 응답은 500, 서버 로그에는 `psycopg2.errors.ForeignKeyViolation: ... violates foreign key constraint "reward_points_user_id_fkey" ... Key (id)=(2) is still referenced from table "reward_points"`가 그대로 찍혔다. 삭제 시도 후 `users`/`reward_points`/`admin_logs`를 다시 조회하니 전부 삭제 전 상태 그대로였다 — `db.commit()` 하나에 묶인 트랜잭션이라 `AdminLog` insert까지 포함해 통째로 롤백됐고, 부분 삭제는 남지 않았다.
4. **카테고리 리맵은 여전히 도는가**: 돈다. 에러 없이 실행되고 `posts.tags[0]`과 `challenges.categories` 원소가 갱신된다. 시드한 `["땀 흘리는 운동","웨이트"]`가 `["일상","웨이트"]`로 나와 `tags[1:]` 보존도 함께 확인됐다. 매핑에 없는 자유 입력 값, NULL, 빈 배열, 파싱 실패 문자열은 전부 손대지 않고 넘어간다.

**B안의 실제 비용**: 위 3번이 실측으로 확인됐으니, B안(DROP 연기)을 택하면 이 FK 문제를 반드시 같이 풀어야 한다. `reward_points.user_id` FK에 `ondelete=CASCADE`(또는 `SET NULL`)를 추가하는 마이그레이션을 하나 더 얹거나, `admin.py`의 유저 삭제 경로에서 `reward_points` 행을 먼저 지우는 코드를 되살리거나 — 방법은 두 가지지만 둘 다 코드 변경이 필요하다. B안은 "코드는 그대로 두고 마이그레이션만 미룬다"는 원래 그림보다, 실제로는 이 트랜잭션 하나를 위해 코드를 한 군데 더 손대야 하는 안이다.

이번처럼 코드 삭제와 DROP이 한 커밋에 이미 묶인 상황에는 D가 창 자체를 없애는 방법이고, 당장 커밋을 다시 쪼갤 여유가 없다면 A로 진행하는 것도 현실적인 선택이다. B를 고른다면 위 FK 수정이 세트로 따라붙는다는 점을 감안해야 한다. 다만 무엇을 택할지, 혹은 다른 조합을 쓸지는 실제 트래픽 패턴과 재작업 여유를 아는 쪽에서 정할 일이다.
