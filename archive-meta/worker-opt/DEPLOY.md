# Orange Story Worker - 배포 가이드

> **[아카이브 — 사용하지 않는 경로]**
>
> 이 문서는 `/opt/stackhealth-worker`에 워커를 **단일 전용 서버로** 올리는 절차다.
> 현재 운영은 이 방식을 쓰지 않는다 — 워커는 앱과 같은 서버에서 systemd `--user`
> 템플릿 유닛 `stack-health-worker@{1..N}`으로 돌고, `scripts/deploy.sh`가 push 배포 때
> 함께 재시작한다. 운영 서버에 `/opt/stackhealth-worker` 디렉토리는 존재하지 않으며
> 어떤 자동화도 이 문서의 명령을 실행하지 않는다.
>
> **실제 워커 운영·배포는 `CLAUDE.md`의 "워커 멀티 인스턴스" 절을 본다.**
> 아래 내용은 전용 서버로 분리할 일이 생겼을 때의 참고용으로만 남긴다.
