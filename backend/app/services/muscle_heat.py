...
POSE_STRIDE = 1  # N프레임마다만 mediapipe 포즈 추론을 실행하고, 건너뛴 프레임은 마지막 근육
                 # 캔버스를 그대로 유지(freeze)해 렌더한다(`_RenderState.step(..., hold=True)`).
                 #
                 # 1 = 매 프레임 추론(기본). 한때 2로 뒀다가 되돌렸다 — hold 프레임은 히트맵을
                 # "이전 검출 시점의 랜드마크 위치"로 그린 캔버스를 재사용하므로, 프레임의 절반이
                 # 실제 몸보다 1프레임(30fps 기준 33ms) 뒤처진다. 줄넘기·버피처럼 팔다리가 빠른
                 # 동작에서는 그 사이 수십 픽셀이 움직여 히트맵이 근육에서 벗어나 보이고,
                 # 사람 픽셀만 남기는 세그멘테이션 컷도 이전 프레임 기준이라 몸 밖으로 삐져나온다.
                 # 속도 이득은 "사람이 없는" 클립으로 측정해 과대평가했던 것으로(검출이 계속
                 # 실패하면 _muscle_layer/세그 처리가 아예 안 돌아 stride 차이가 거의 없다),
                 # 실측 재확인 결과 이 조건에서 2 vs 1 차이는 2.5%에 불과했다.
                 # 다시 올리려면 반드시 사람이 나오는 실사 클립으로 이득을 재고, hold 프레임의
                 # 위치 지연을 랜드마크 외삽 등으로 보정한 뒤에 해야 한다.
                 #
